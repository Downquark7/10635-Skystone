...
//            telemetry.addData("color", color == 1 ? "red" : "blue");
        ...
        drive.turn(2*Math.PI);

//  scan for stone order
        Position order = Position.Unknown;//vuforia.getPosition(-19.8, -13, -2.8);//get numbers using getVuforiaNumbers teleop
        double left = -19.8;
        double center = -13;
        double right = -2.8;
        vuforia.vuforiaTimeout.reset();
        while (vuforia.vuforiaTimeout.seconds() < 5 || drive.isBusy()) {
            drive.update();
            double data = vuforia.getData(false);
...
// strafe towards chosen skystone
        ...
        switch (order) {
            case Left:
            case Center:
                drive.followTrajectorySync(drive.trajectoryBuilder().splineTo(new Pose2d(31, 52 - blockWidth * 1, 0.55 + Math.PI)).forward(10).build());
                break;
            case Right:
                drive.followTrajectorySync(drive.trajectoryBuilder().splineTo(new Pose2d(31, 52 - blockWidth * 2, 0.55 + Math.PI)).forward(10).build());
                break;
            default:
                sleep(2000);
                drive.followTrajectorySync(drive.trajectoryBuilder().splineTo(new Pose2d(31, 52 - blockWidth * 0 * Math.random(), 0.55 + Math.PI)).forward(10).build());
        }
//        drive.followTrajectorySync(drive.trajectoryBuilder().forward(6).build());
        drive.Gripper.setPosition(drive.GripperClosed);
        drive.followTrajectorySync(drive.trajectoryBuilder().back(17).setReversed(true).splineTo(new Pose2d(40, -10, Math.PI / 2)).build());
