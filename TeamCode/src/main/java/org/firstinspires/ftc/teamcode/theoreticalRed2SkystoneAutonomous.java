...
@Autonomous
public class theoreticalRed2SkystoneAutonomous extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        drive.setPoseEstimate(new Pose2d(-63.5, 35, 0));               // Tell the program where the robot is starting on the field
        ...
        switch (order) {
            case Left:
                drive.followTrajectorySync(drive.trajectoryBuilder().splineTo(new Pose2d(-31,  57 - blockWidth * 0, 0.55)).forward(6).build());    //  needs to be changed for far stones
                break;
            case Center:
                drive.followTrajectorySync(drive.trajectoryBuilder().splineTo(new Pose2d(-31, 57 - blockWidth * 1, 0.55)).forward(6).build());    //  needs to be changed for far stones
                break;
            case Right:
                drive.followTrajectorySync(drive.trajectoryBuilder().splineTo(new Pose2d(-31, 57 - blockWidth * 2, 0.55)).forward(6).build());    //  needs to be changed for far stones
                break;
            default:
        ...
        while (robot.LeftLift.getCurrentPosition() > 0){    // wait for Lift to go down before going under bridge
            drive.update();
        }
        ...
        drive.followTrajectorySync(drive.trajectoryBuilder().setReversed(true).splineTo(new Pose2d(-27, -46, Math.PI)).build());    // drive to platform   to get ready to grab it and drop stone
